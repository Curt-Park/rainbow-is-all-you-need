...
def _():
    ...
    return F, gym, nn, np, optim, os, plt, random, torch, warnings
...
def _(mo):
    ...
    The key data structure behind PER is the *Segment Tree*, which allows O(log n) priority updates and O(log n) proportional sampling. It stores priorities in a binary tree where each parent node holds the sum (or min) of its children.

    We implement three classes:

    - **SegmentTree**: Base class that supports any associative binary operation (sum, min, etc.) over a fixed-size array with O(log n) updates and range queries.
    - **SumSegmentTree**: Stores cumulative priorities for stratified proportional sampling. The `retrieve` method finds the transition corresponding to a sampled priority value.
    - **MinSegmentTree**: Tracks the minimum priority across all transitions, used to compute the maximum importance-sampling weight for normalization.
...
def _(mo):
    ...
    `PrioritizedReplayBuffer` extends the basic `ReplayBuffer` with priority-based sampling using the segment trees defined above.

    Key methods:

    - **store**: Stores a new transition with maximum priority, ensuring it gets sampled at least once.
    - **sample_batch**: Samples a batch using stratified proportional sampling based on priorities. Returns importance-sampling weights (controlled by $\beta$) to correct for the non-uniform sampling bias.
    - **update_priorities**: Updates the priorities of sampled transitions based on their latest TD errors.
    """)
    return
